 ...
 */
function removeDisallowedElements(element: HTMLElement, allowedTags: string[]): void {
  ...
  for (const child of children) {
    if (!allowedTags.includes(child.tagName.toLowerCase())) {
      ...
      removeDisallowedElements(child as HTMLElement, allowedTags);
    }
  }
}
...
function removeEventHandlers(element: HTMLElement): void {
  ...
  for (const child of Array.from(element.children)) {
    removeEventHandlers(child as HTMLElement);
  }
}
